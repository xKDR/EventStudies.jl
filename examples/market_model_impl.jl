...
linecolors = Makie.PlotUtils.distinguishable_colors(11; lchoices = LinRange(50, 100, 5))
f = Figure()
a = Axis(f[1, 1])
i = 10
for colname in names(plot_ts)
    lines!(a, Dates.value.(index(plot_ts)), remap_cumsum(plot_ts).coredata[!, colname] .- i; color = linecolors[i ÷ 10],  label = string(colname))
    i += 10
end
Legend(f[1, 2], a)
